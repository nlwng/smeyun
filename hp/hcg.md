...
3.oam网络
4.数据网络
5.pxe boot网络，上联为tagged可与管理网络共享(请一定设置否则无法打补丁)
6.oam网络可以和存储网络合并
```
...
(只有当网络类型设置为pci-sriov时才显示)可用的虚拟接口的最大数量。


更改接口设置的能力对于更新管理非常有用接口。当一个计算节点首先创建时，它的内部管理接口是自动设置使用默认接口类型(以太网)。
如果你在使用滞后内部管理网络，您必须手动将此更新为聚合以太网。您还可以编辑内部管理或基础设施接口来连接到数据网络通过选择
数据作为接口的附加网络类型，或者使用CLI。对于CLI 指示,

note:
1.不能使用VLAN数据接口附加到VLAN提供程序网络。

Balanced XOR
提供聚合带宽和错误宽容。使用相同的从属接口每个目的地的MAC地址。此模式使用默认的传输策略，确定目标从属接口的位置通过计算源MAC地址XOR
有目的地MAC地址，模2。您可以使用此修改传输策略 xmit-hash-policy选项
802.3ad
